 ...
 */
@NoRepositoryBean
public interface BaseSalOrderControlRepository extends SalOrderControlRepositoryCustom, PagodaJpaRepository<SalOrderControl,Long>  {
     ...
     * @return
     */
    @SqlTemplate(name="selectWithTime" , sql="select * from sal_order_control where {{#place_order_time}}   place_order_time = :place_order_time{{/place_order_time}}{{#idList_exists}}    and id in ({{#idList}}{{^-first}}, {{/-first}}{{this}}{{/idList}}){{/idList_exists}}")
    Page<SalOrderControlDTO> selectWithTime(@Param("place_order_time") null placeOrderTime, @Param("idList") List<Long> idList,  @Param("pageable") Pageable pageable);
        ...
        */
        @SqlTemplate(name="selectWithTime" , sql="select * from sal_order_control where {{#place_order_time}}   place_order_time = :place_order_time{{/place_order_time}}{{#idList_exists}}    and id in ({{#idList}}{{^-first}}, {{/-first}}{{this}}{{/idList}}){{/idList_exists}}")
        List<SalOrderControlDTO> selectWithTime(@Param("place_order_time") null placeOrderTime, @Param("idList") List<Long> idList );
