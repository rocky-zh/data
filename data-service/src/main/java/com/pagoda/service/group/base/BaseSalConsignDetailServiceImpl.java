...
/**
 * sal_consign_detail服务实现类SalConsignDetailServiceImpl继承的父类,实现一些平台共用的方法
 *
...
public abstract class BaseSalConsignDetailServiceImpl
  ...
  @Autowired protected SalConsignDetailTreeRepository treeRepository;
  ...
  @Override
  public SalConsignDetailDTO createLeafNode(
      @ApiParam(value = "entity", required = true) @Valid SalConsignDetailDTO entity,
    ...
    SalConsignDetailDTO dto = repository.create(SalConsignDetail.convertDTO(entity));

    SalConsignDetailTree leaf = new SalConsignDetailTree();
    ...
    List<SalConsignDetailTree> parents = treeRepository.getParents(pid, 0L, catId, entId);
    if (parents != null) {
      List<SalConsignDetailTree> relations = new ArrayList<>(parents.size());
      for (SalConsignDetailTree t : parents) {
        SalConsignDetailTree rel = new SalConsignDetailTree();
        rel.setPid(t.getPid());
  ...
  public void moveSubTree(
    ...
    //  按节点距离递减，第一个是根节点
    List<SalConsignDetailTree> oldParents = treeRepository.getParents(cid, 1L, catId, entId);
    // 按节点距离递增，最后一个是叶子节点
    List<SalConsignDetailTree> children = treeRepository.getTreeNodes(cid, catId, entId);

    children.add(new SalConsignDetailTree(cid, cid, 0L, catId, entId));
    ...
    List<SalConsignDetailTree> parents = treeRepository.getParents(pid, 1L, catId, entId);
    ...
    for (int p = parentIds.size() - 1; p >= 0; p--) {
      ...
      for (SalConsignDetailTree c : children) {
        SalConsignDetailTree leaf = new SalConsignDetailTree();
  ...
  public void deleteLeafNode(
      ...
      @ApiParam(value = "entId", required = true) Long entId)
      throws ServiceException {
    List<SalConsignDetailTree> children = treeRepository.getChildren(id, catId, entId);
  ...
  public void deleteSubTree(
      ...
      @ApiParam(value = "entId", required = true) Long entId) {
    // 查询子树上的所有节点
    List<SalConsignDetailTree> nodes = treeRepository.getTreeNodes(id, catId, entId);
  ...
  @Override
  public SalConsignDetailDTO getParentNode(
      @ApiParam(value = "id", required = true) Long id,
      @ApiParam(value = "catId", required = true) Long catId,
      @ApiParam(value = "entId", required = true) Long entId)
      throws ServiceException {
  ...
  @Override
  public Iterable<SalConsignDetailDTO> getChildren(
      @ApiParam(value = "id", required = true) Long id,
      @ApiParam(value = "catId", required = true) Long catId,
      @ApiParam(value = "entId", required = true) Long entId)
      throws ServiceException {
  ...
  @Override
  public List<SalConsignDetailDTO> getLeafNodes(
      @ApiParam(value = "id", required = true) Long id,
      @ApiParam(value = "catId", required = true) Long catId,
      @ApiParam(value = "entId", required = true) Long entId)
      throws ServiceException {
    try {
      SalConsignDetailDTO node = getById(id);
      if (node == null) {
      ...
      // 查询非叶子节点
      List<SalConsignDetailTree> parentNodes = treeRepository.getChildren(idList, catId, entId);
      if (parentNodes != null) {
  ...
  @Override
  public SalConsignDetailDTO getTree(
      @ApiParam(value = "id", required = true) Long id,
      @ApiParam(value = "catId", required = true) Long catId,
      @ApiParam(value = "entId", required = true) Long entId)
      throws ServiceException {
    try {
      ...
      // 将所有节点放入map中，方便查询
      List<SalConsignDetail> chidrenNodes = repository.findAll(ids);
      Map<Long, SalConsignDetailDTO> nodeMap =
          chidrenNodes.stream().collect(Collectors.toMap(SalConsignDetail::getId, u -> u.toDTO()));
      nodeMap.put(id, node);

      // 遍历父子关系，构建树形结构
      List<SalConsignDetailTree> relation = treeRepository.getParentChild(ids, catId, entId);
      for (SalConsignDetailTree r : relation) {
        SalConsignDetailDTO parent = nodeMap.get(r.getPid());
        SalConsignDetailDTO child = nodeMap.get(r.getCid());
