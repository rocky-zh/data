 ...
 */
@NoRepositoryBean
public interface BasePersonRepository extends PersonRepositoryCustom, PagodaJpaRepository<Person,Long>  {
     ...
     * @return
     */
    @SqlTemplate(name="selectWithTime" , sql="select * from person where {{#order_time}} order_time= :order_time{{/order_time}}{{#idList_exists}} and id in ({{#idList}}{{^-first}}, {{/-first}}{{this}}{{/idList}}){{/idList_exists}}")
    Page<PersonDTO> selectWithTime(@Param("order_time") null orderTime, @Param("idList") List<Long> idList,  @Param("pageable") Pageable pageable);
        ...
        */
        @SqlTemplate(name="selectWithTime" , sql="select * from person where {{#order_time}} order_time= :order_time{{/order_time}}{{#idList_exists}} and id in ({{#idList}}{{^-first}}, {{/-first}}{{this}}{{/idList}}){{/idList_exists}}")
        List<PersonDTO> selectWithTime(@Param("order_time") null orderTime, @Param("idList") List<Long> idList );
