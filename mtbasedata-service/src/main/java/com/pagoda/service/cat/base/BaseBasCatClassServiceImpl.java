...
/**
 * 目录分类表:全系统多维度树形结构表[包含：机构，供应商，部门，客户，库房...等] 1-level：
 * tab服务实现类BasCatClassServiceImpl继承的父类,实现一些平台共用的方法
 *
...
public abstract class BaseBasCatClassServiceImpl implements BasCatClassService, InitializingBean {
  ...
  @ApiOperation(value = "searchCatClassRelBindData", notes = "查找分类关系表中目录分类是否绑定数据")
  @Override
  public Page<BasCatClassDTO> searchCatClassRelBindData(
      ...
      throws ServiceException {
    try {
      return repository.searchCatClassRelBindData(catClassId, deleted, idList, pageable);
    } catch (Exception e) {
      throw new ServiceException(e);
    }
  }

  @ApiOperation(value = "basCatClassFuzzyQuery", notes = "分页模糊查询目录分类")
  @Override
  public Page<BasCatClassDTO> basCatClassFuzzyQuery(
      @ApiParam("id") Long id,
      ...
      throws ServiceException {
    try {
      return repository.basCatClassFuzzyQuery(
          id,
          catId,
          code,
          createdAt,
  ...
  @ApiOperation(value = "findTopBasCatClassByCatid", notes = "根据目录id查找直接(顶级)目录分类列表")
  @Override
  public Page<BasCatClassDTO> findTopBasCatClassByCatid(
      @ApiParam("cat_id") Long catId,
      @ApiParam("deleted") Long deleted,
      @ApiParam("ent_id") Long entId,
      @ApiParam("id_list") List<Long> idList,
      ...
      throws ServiceException {
    try {
      return repository.findTopBasCatClassByCatid(catId, deleted, entId, idList, pageable);
    } catch (Exception e) {
      throw new ServiceException(e);
    }
  }

  @ApiOperation(value = "setFatherClass", notes = "调整目录分类的父分类")
  @Override
  @Transactional(rollbackFor = ServiceException.class)
  public Integer setFatherClass(
      @ApiParam("parent_cat_class_name") String parentCatClassName,
      @ApiParam("parent_cat_class_code") String parentCatClassCode,
      @ApiParam("id") Long id,
      @ApiParam("deleted") Long deleted,
      @ApiParam("parent_cat_class_id") Long parentCatClassId) {
    try {
      return repository.setFatherClass(
          parentCatClassName, parentCatClassCode, id, deleted, parentCatClassId);
    } catch (Exception e) {
      throw new ServiceException(e);
    }
  }
