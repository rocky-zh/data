...
public class BasGoodsDTO extends AbstractDTO implements Serializable {
  ...
  public static ValidatorBuilder<BasGoodsDTO> validatorExample() {
    ...
    builder.field(_goodsType, notNull, "商品类型{可选值集数据字典}为空");
    builder.field(_invUnitId, notNull, "库存单位ID为空");
    builder.field(_invUnitCode, notNull, "库存单位代码[冗余]为空");
    builder.field(_invRate, notNull, "库存单位转换比(库存单位转为基础单位转换比)为空");
    builder.field(_isEntLot, notNull, "是否管理批次(企业级)为空");
    builder.field(_shelfLifeQty, notNull, "保质期数量为空");
    builder.field(_code, notNull, "行政单位代码为空");
    builder.field(_entId, notNull, "企业id为空");
    builder.field(_basUnitId, notNull, "基础计量单位ID为空");
    builder.field(_basUnitCode, notNull, "基础计量单位代码[冗余]为空");
    builder.field(_basUnitName, notNull, "基本单位名称[冗余]为空");
    builder.field(_salUnitName, notNull, "销售单位名称[冗余]为空");
    builder.field(_isEnabled, notNull, "是否可用[设置为不可用，需要把bas_org_productvar 下也设置为不可用]为空");
    builder.field(_isPur, notNull, "是否可订(采购)[采购按主商品采购]为空");
    builder.field(_isSale, notNull, "是否允许销售(b2b用)为空");
    builder.field(_isRet, notNull, "是否零售(b2c用，另因按订单退，故无需是否允许退货)为空");
    return builder;
  }
