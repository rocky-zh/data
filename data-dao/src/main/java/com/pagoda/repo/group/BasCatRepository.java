...
public interface BasCatRepository
  ...
  @SqlTemplate(
    name = "getParents",
    sql =
        "SELECT u.* FROM `bas_cat` u, `bas_cat_tree` t WHERE u.id=t.pid AND t.cid=:id AND t.cat_id=:catId AND u.deleted=0 {{#entId}} AND t.ent_id=:entId{{/entId}} ORDER BY t.distance DESC"
  )
  List<BasCatDTO> getParents(
  ...
  @SqlTemplate(
    name = "getParentNode",
    sql =
        "SELECT u.* FROM `bas_cat` u, `bas_cat_tree` t WHERE u.id=t.pid AND t.cid=:id AND t.distance=1 AND t.cat_id=:catId AND u.deleted=0 {{#entId}} AND t.ent_id=:entId{{/entId}}"
  )
  ...
  @SqlTemplate(
    name = "getChildren",
    sql =
        "SELECT u.* FROM `bas_cat` u, `bas_cat_tree` t WHERE u.id=t.cid AND t.pid=:id AND t.distance=1 AND t.cat_id=:catId AND u.deleted=0 {{#entId}} AND t.ent_id=:entId{{/entId}}"
  )
  List<BasCatDTO> getChildren(
      @Param("id") Long id, @Param("catId") Long catId, @Param("entId") Long entId);
  ...
  @SqlTemplate(
    name = "getTreeNodes",
    sql =
        "SELECT u.* FROM `bas_cat` u, `bas_cat_tree` t WHERE u.id=t.cid AND t.pid=:id AND t.distance > 0 AND t.cat_id=:catId AND u.deleted=0 {{#entId}} AND t.ent_id=:entId{{/entId}} ORDER BY t.distance ASC"
  )
  List<BasCatDTO> getTreeNodes(
      @Param("id") Long id, @Param("catId") Long catId, @Param("entId") Long entId);
}
