...
public interface BasCatClassTreeRepository extends PagodaJpaRepository<BasCatClassTree, Long> {
  ...
  @SqlTemplate(
    name = "deleteLeafNode",
    sql =
        "DELETE FROM `bas_cat_class_tree` WHERE cid=:id AND cat_id=:catId {{#entId}} AND ent_id=:entId{{/entId}}"
  )
  @Modifying
  ...
  @SqlTemplate(
    ...
    sql =
        "DELETE FROM `bas_cat_class_tree` WHERE pid in ({{#parents}}{{^-first}}, {{/-first}}{{.}}{{/parents}}) AND cid in ({{#children}}{{^-first}}, {{/-first}}{{.}}{{/children}}) AND cat_id=:catId {{#entId}} AND ent_id=:entId{{/entId}}"
  )
  @Modifying
  ...
  @SqlTemplate(
    name = "deleteSubTree",
    sql =
        "DELETE FROM `bas_cat_class_tree` WHERE cid in ({{#idList}}{{^-first}}, {{/-first}}{{.}}{{/idList}}) AND cat_id=:catId {{#entId}} AND ent_id=:entId{{/entId}}"
  )
  @Modifying
  void deleteSubTree(
  ...
  @SqlTemplate(
    name = "getParentNode",
    sql =
        "SELECT * FROM `bas_cat_class_tree` WHERE cid=:id AND cat_id=:catId AND distance=1 {{#entId}} AND ent_id=:entId{{/entId}}"
  )
  ...
  @SqlTemplate(
    name = "getParents",
    sql =
        "SELECT * FROM `bas_cat_class_tree` t WHERE t.cid=:id AND t.cat_id=:catId AND t.distance>= :distance {{#entId}} AND t.ent_id=:entId{{/entId}} ORDER BY t.distance DESC"
  )
  ...
  @SqlTemplate(
    name = "getChildren",
    sql =
        "SELECT * FROM `bas_cat_class_tree` t WHERE t.pid=:id AND t.distance=1 AND t.cat_id=:catId {{#entId}} AND t.ent_id=:entId{{/entId}}"
  )
  ...
  @SqlTemplate(
    name = "getChildren",
    sql =
        "SELECT * FROM `bas_cat_class_tree` t WHERE t.pid IN ({{#idList}}{{^-first}}, {{/-first}}{{.}}{{/idList}}) AND t.distance=1 AND t.cat_id=:catId {{#entId}} AND t.ent_id=:entId{{/entId}}"
  )
  List<BasCatClassTree> getChildren(
      @Param("idList") List<Long> idList, @Param("catId") Long catId, @Param("entId") Long entId);
  ...
  @SqlTemplate(
    name = "getTreeNodes",
    sql =
        "SELECT * FROM `bas_cat_class_tree` t WHERE t.pid=:id AND t.distance > 0 AND t.cat_id=:catId {{#entId}} AND t.ent_id=:entId{{/entId}} ORDER BY t.distance ASC"
  )
  ...
  @SqlTemplate(
    name = "getParentChild",
    sql =
        "SELECT * FROM `bas_cat_class_tree` t WHERE t.cid in ({{#idList}}{{^-first}}, {{/-first}}{{.}}{{/idList}}) AND t.distance = 1 AND t.cat_id=:catId {{#entId}} AND t.ent_id=:entId{{/entId}}"
  )
