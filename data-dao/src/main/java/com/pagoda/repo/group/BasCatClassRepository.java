...
public interface BasCatClassRepository
  ...
  @SqlTemplate(
    name = "getParents",
    sql =
        "SELECT u.* FROM `bas_cat_class` u, `bas_cat_class_tree` t WHERE u.id=t.pid AND t.cid=:id AND t.cat_id=:catId AND u.deleted=0 {{#entId}} AND t.ent_id=:entId{{/entId}} ORDER BY t.distance DESC"
  )
  ...
  @SqlTemplate(
    name = "getParentNode",
    sql =
        "SELECT u.* FROM `bas_cat_class` u, `bas_cat_class_tree` t WHERE u.id=t.pid AND t.cid=:id AND t.distance=1 AND t.cat_id=:catId AND u.deleted=0 {{#entId}} AND t.ent_id=:entId{{/entId}}"
  )
  ...
  @SqlTemplate(
    name = "getChildren",
    sql =
        "SELECT u.* FROM `bas_cat_class` u, `bas_cat_class_tree` t WHERE u.id=t.cid AND t.pid=:id AND t.distance=1 AND t.cat_id=:catId AND u.deleted=0 {{#entId}} AND t.ent_id=:entId{{/entId}}"
  )
  ...
  @SqlTemplate(
    name = "getTreeNodes",
    sql =
        "SELECT u.* FROM `bas_cat_class` u, `bas_cat_class_tree` t WHERE u.id=t.cid AND t.pid=:id AND t.distance > 0 AND t.cat_id=:catId AND u.deleted=0 {{#entId}} AND t.ent_id=:entId{{/entId}} ORDER BY t.distance ASC"
  )
